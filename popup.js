// popup.js
// Main popup logic for The Collector extension
...
const IMAGE_EXTENSIONS = window.Constants
  ? window.Constants.IMAGE_EXTENSIONS
  : [".jpg", ".jpeg", ".png", ".gif", ".webp", ".svg"];
const GROUP_COLORS = window.Constants
  ? window.Constants.GROUP_COLORS
  : [
      "#1a73e8",
      "#ea4335",
      "#34a853",
...
const THEME_PRESETS = window.ThemeManager
  ? window.ThemeManager.THEME_PRESETS
  : {};
const THEME_SCHEMA = window.ThemeManager
  ? window.ThemeManager.THEME_SCHEMA
  : { properties: {} };
...
function updateFilename(url, newName, source) {
  ...
  const item = document.querySelector(
    '.image-item[data-url="' + CSS.escape(url) + '"]'
  );
  if (item) {
  ...
  if (
    source !== "download" &&
    typeof pendingDownloads !== "undefined" &&
    pendingDownloads
  ) {
    const downloadItem = pendingDownloads.find(function (d) {
      return d.url === url;
    });
      ...
      if (downloadModal && downloadModal.classList.contains("visible")) {
        ...
        setTimeout(function () {
          if (
            ...
            typeof buildTreeStructure === "function" &&
            typeof renderDownloadTree === "function"
          ) {
...
function removeImageCentralized(url) {
  ...
  if (typeof pendingDownloads !== "undefined" && pendingDownloads) {
    const hadDownload = pendingDownloads.some(function (d) {
      return d.url === url;
    });
    if (hadDownload) {
      pendingDownloads = pendingDownloads.filter(function (d) {
        return d.url !== url;
      });
      // Re-render download tree if modal is visible
      const downloadModal = document.getElementById("download-preview-modal");
  ...
  browserAPI.storage.local.get("navigationStack", function (result) {
    ...
    const newStack = stack.filter(function (u) {
      return u !== url;
    });
...
function removeImagesBulk(urls) {
  ...
  if (typeof pendingDownloads !== "undefined" && pendingDownloads) {
    const hadDownloads = pendingDownloads.some(function (d) {
      return urlSet.has(d.url);
    });
    if (hadDownloads) {
      pendingDownloads = pendingDownloads.filter(function (d) {
        return !urlSet.has(d.url);
...
function updateGroupDirectory(groupId, directory, source) {
  ...
  groups.forEach(function (group) {
    const section = document.querySelector(
      '.group-section[data-group-id="' + group.id + '"]'
    );
    if (section) {
    ...
    if (downloadModal && downloadModal.classList.contains("visible")) {
      ...
      setTimeout(function () {
        ...
        pendingDownloads.forEach(function (download) {
          const downloadGroup = groups.find(function (g) {
            return g.urls.includes(download.url);
          });
          if (downloadGroup) {
...
function applySettingsToUI() {
  ...
  if (filenameTemplate)
    filenameTemplate.value = settings.filenameTemplate || "{name}";
  ...
  if (listThumbnailSize) {
    ...
    if (listThumbnailValue)
      listThumbnailValue.textContent = listThumbnailSize.value + "px";
  }
  ...
  if (gridThumbnailSize) {
    ...
    if (gridThumbnailValue)
      gridThumbnailValue.textContent = gridThumbnailSize.value + "px";
  }

  const showDimensions = document.getElementById("show-dimensions");
  if (showDimensions)
    showDimensions.checked = settings.showDimensions !== false;
  ...
  if (rememberGroups)
    rememberGroups.checked = settings.rememberGroups !== false;
...
function applyUIScale(scale) {
  ...
  root.setAttribute(
    "data-ui-scale",
    validScales.includes(scale) ? scale : "medium"
  );
}
...
function applyDensity(density) {
  ...
  root.setAttribute(
    "data-density",
    validDensities.includes(density) ? density : "comfortable"
  );
}
...
function setupSettingsListeners() {
  ...
  if (listThumbnailSlider) {
    listThumbnailSlider.addEventListener("input", function () {
      if (listThumbnailValue)
        listThumbnailValue.textContent = this.value + "px";
  ...
  if (gridThumbnailSlider) {
    gridThumbnailSlider.addEventListener("input", function () {
      if (gridThumbnailValue)
        gridThumbnailValue.textContent = this.value + "px";
  ...
  if (applyCustomThemeBtn && customThemeInput) {
    applyCustomThemeBtn.addEventListener("click", function () {
      ...
      if (result.valid) {
        ...
        saveSettings();
        if (customThemeError) {
          customThemeError.textContent = result.theme
            ? "Custom overrides applied!"
            : "Custom overrides cleared.";
...
function createImageItem(url, existingNames) {
  ...
  if (failedUrls.has(url)) {
    ...
    img.addEventListener("load", function () {
      ...
      if (metaEl && settings.showDimensions) {
        metaEl.textContent = formatDimensions(
          img.naturalWidth,
          img.naturalHeight
        );
      }
  ...
  if (settings.showDimensions) {
    ...
    dimensions.textContent = cached
      ? formatDimensions(cached.width, cached.height)
      : "...";
...
function showPreviewModal(url) {
  ...
  dimensions.textContent = meta.width
    ? formatDimensions(meta.width, meta.height) + " px"
    : "Loading...";
...
function buildDownloadList(options) {
  ...
  const groupId =
    options.groupId !== undefined ? options.groupId : currentDownloadGroupId;
  const includeUngrouped =
    options.includeUngrouped !== undefined
      ? options.includeUngrouped
      : includeUngroupedInDownload;
  ...
  if (groupId) {
    ...
    if (scope === "selected") {
      ...
      urlsToProcess = group.urls.filter(function (url) {
    ...
    const groupedUrls = groups.flatMap(function (g) {
      return g.urls;
    });
  ...
  urlsToProcess.forEach(function (url) {
    ...
    if (
      ...
      const groupName = group
        ? group.name
        : settings.ungroupedDirectory || "Ungrouped";
...
function renderDownloadTree(tree) {
  ...
  sortedDirs.forEach((dir) => {
    ...
    tree[dir].forEach((file) => {
      ...
      if (file.hasConflict) {
        ...
        toggleBtn.className =
          "tree-toggle-btn icon-btn icon-btn-ghost icon-btn-sm";
        toggleBtn.title = willRename
          ? "Click to overwrite instead"
          : "Click to auto-rename instead";
        ...
        toggleBtn.addEventListener("click", function (e) {
          ...
          pendingDownloads[file.index].willRename =
            !pendingDownloads[file.index].willRename;
          // Re-render the tree
  ...
  if (legend && hasAnyConflicts) {
    if (hasOverwriteConflicts) {
      legend.innerHTML =
        '<span class="legend-item conflict">⚠ Some files will overwrite existing files</span>';
    } else {
      legend.innerHTML =
        '<span class="legend-item rename">↻ Duplicate filenames will be auto-renamed</span>';
    }
  }
...
function startInlineRename(fileEl, index, filenameSpan) {
  ...
  const extension = extMatch ? extMatch[1] : "";
  const nameWithoutExt = extension
    ? currentFilename.slice(0, -extension.length)
    : currentFilename;
...
function updatePreviewSummary() {
  ...
  if (summary && pendingDownloads.length > 0) {
    const groupCount = new Set(
      pendingDownloads.map((d) => d.directory || "(root)")
    ).size;
    summary.textContent =
...
function refreshDownloadPreview() {
  ...
  if (downloads.length === 0) {
    ...
    document.getElementById("tree-container").innerHTML =
      '<div style="color: var(--text-muted); text-align: center; padding: 20px;">No images match the current selection</div>';
  ...
  const groupCount = new Set(downloads.map((d) => d.directory || "(root)"))
    .size;
  summary.textContent =
    downloads.length +
    " image" +
...
function countSelectedInGroup(groupId) {
  ...
  if (!group) return 0;
  return group.urls.filter(function (url) {
    return selectedUrls.has(url);
  }).length;
}
...
function updateUngroupedOption() {
  ...
  const ungroupedCountSpan = document.getElementById(
    "ungrouped-download-count"
  );
  const ungroupedUrls = getUngroupedUrls();
  ...
  if (currentDownloadScope === "selected" && selectedUrls.size > 0) {
    ungroupedCount = ungroupedUrls.filter(function (url) {
      return selectedUrls.has(url);
    }).length;
  } else {
...
function showDownloadPreviewForSingleImage(url, itemElement) {
  ...
  const filename =
    (input ? input.value : "image") + (extSpan ? extSpan.textContent : "");
...
function setupDropSelectionModal() {
  ...
  const selectRecommendedBtn = document.getElementById(
    "drop-select-recommended"
  );
  ...
  function updateSelectionCount() {
    ...
    const selectedCount = pendingDropItems.items.filter(
      (i) => i.selected
    ).length;
    const countSpan = document.getElementById("drop-select-count");
    countSpan.textContent =
      selectedCount + " of " + pendingDropItems.items.length + " selected";
  ...
  function setRecommendedSelection() {
    ...
    pendingDropItems.items.forEach(function (item, idx) {
      ...
      item.selected =
        ...
        item.hint !== ITEM_HINTS.UI_ELEMENT;
      updateItemUI(idx);
    });
  }
  ...
  confirmBtn.addEventListener("click", function () {
    ...
    const selectedItems = pendingDropItems.items.filter(function (i) {
      return i.selected;
    });
...
function renderDropSelectionItems(dropData) {
  ...
  dropData.items.forEach(function (item) {
    if (item.selected === undefined) {
      item.selected =
        item.hint === ITEM_HINTS.PRIMARY || item.hint === ITEM_HINTS.UNKNOWN;
    }
  });
...
function updateDropSelectionCount() {
  ...
  const selectedCount = pendingDropItems.items.filter(function (i) {
    return i.selected;
  }).length;
...
function addSelectedDropItems(items, groupId) {
  ...
  // Add to storage
  browserAPI.storage.local.get(
    ["navigationStack", "groups", "urlMeta"],
    function (result) {
      const stack = result.navigationStack || [];
      ...
      newUrls.forEach(function (url) {
        ...
        if (groupId) {
          const group = storedGroups.find(function (g) {
            return g.id === groupId;
          });
      ...
      browserAPI.storage.local.set(
        ...
        function () {
          ...
          if (newUrls.length === 1) {
            ...
            showDropFeedback(
              "Added " + newUrls.length + " images to " + groupName,
              "success"
            );
          }
        }
...
function handleDropWithSelection(e, groupId) {
  ...
  const nonRecommended = imageItems.filter(function (item) {
    return (
      item.hint === ITEM_HINTS.DUPLICATE || item.hint === ITEM_HINTS.UI_ELEMENT
    );
  });
  ...
  showDropSelectionModal(
    { items: imageItems, recommended: dropData.recommended },
    groupId
  );
}
...
function renderGroups() {
  ...
  groups.forEach((group) => {
    ...
    const selectedInGroup = group.urls.filter(function (url) {
      return selectedUrls.has(url);
    }).length;
    ...
    const downloadBtn = Icons.createIconButton("download", {
      ...
      disabled: isEmpty,
      onClick: function () {
        showDownloadPreviewForGroup(group.id);
      },
    });
    actions.appendChild(downloadBtn);
...
function createDropGhost(sourceItem, isMultiple) {
  ...
  if (isMultiple && draggedUrls.length > 1) {
    ...
    summary.innerHTML =
      ...
      count +
      '</span><span class="ghost-label">images</span>';
    ...
    clone
      .querySelectorAll("button, input, .item-checkbox")
      .forEach(function (el) {
        el.remove();
      });
...
function handleDragStart(e) {
  ...
  if (isSelected && hasMultipleSelected) {
    ...
    draggedUrls.forEach(function (u) {
      const el = document.querySelector(
        '.image-item[data-url="' + CSS.escape(u) + '"]'
      );
  ...
  e.dataTransfer.setData(
    "application/x-collector-internal",
    draggedUrls.join(",")
  );
}
...
function handleDragEnd() {
  ...
  document
    .querySelectorAll(".group-section, .ungrouped-section")
    .forEach(function (s) {
...
function handleDrop(e) {
  ...
  if (dropTargetItem && dropPosition) {
    ...
    const items = Array.from(
      section.querySelectorAll(".image-item:not(.drop-ghost):not(.dragging)")
    );
...
function showMoveConfirmation(
  ...
  dialog.querySelector(".drag-intent-description").textContent =
    'All selected images were moved to "' +
    targetName +
    '". Did you only want to move one?';
  ...
  const filename =
    imageMeta[primaryUrl]?.name || getFilenameFromUrl(primaryUrl);
  ...
  document
    .getElementById("drag-intent-single")
    .querySelector(".option-title").textContent = "Just this one";
  document
    .getElementById("drag-intent-selected")
    .querySelector(".option-title").textContent = "Keep all";
  ...
  function cleanup() {
    ...
    document
      .getElementById("drag-intent-single")
      .removeEventListener("click", handleMoveSingleOnly);
    document
      .getElementById("drag-intent-selected")
      .removeEventListener("click", handleKeepAll);
    document
      .getElementById("drag-intent-cancel")
      .removeEventListener("click", handleKeepAll);
    ...
    document
      .getElementById("drag-intent-single")
      .querySelector(".option-title").textContent = "Move this image only";
    document
      .getElementById("drag-intent-selected")
      .querySelector(".option-title").textContent = "Move all selected";
  ...
  function handleMoveSingleOnly() {
    ...
    const othersToUndo = movedUrls.filter(function (u) {
      return u !== primaryUrl;
    });

    // Remove others from target group first
    if (targetGroupId) {
      const tg = groups.find(function (g) {
        return g.id === targetGroupId;
      });
      if (tg) {
        tg.urls = tg.urls.filter(function (u) {
          return !othersToUndo.includes(u);
        });
      }
    }
    ...
    othersToUndo.forEach(function (url) {
      ...
      if (originalGroupId) {
        ...
        const originalGroup = groups.find(function (g) {
          return g.id === originalGroupId;
        });
  ...
  document
    .getElementById("drag-intent-single")
    .addEventListener("click", handleMoveSingleOnly);
  document
    .getElementById("drag-intent-selected")
    .addEventListener("click", handleKeepAll);
  document
    .getElementById("drag-intent-cancel")
    .addEventListener("click", handleKeepAll);
}
...
function moveUrlToGroup(url, targetGroupId, insertIndex) {
  ...
  // Find source group
  const sourceGroup = groups.find(function (g) {
    return g.urls.includes(url);
  });
  ...
  if (targetGroup && !targetGroup.urls.includes(url)) {
    ...
    const ungroupedUrls = imageURLs.filter(function (u) {
      return !groupedUrls.has(u) && u !== url;
    });
    ...
    if (insertIndex >= 0 && insertIndex <= ungroupedUrls.length) {
      ...
      const targetPosInImageURLs = targetUrl
        ? imageURLs.indexOf(targetUrl)
        : imageURLs.length;
      ...
      const adjustedPos =
        currentPos !== -1 && currentPos < targetPosInImageURLs
...
function moveUrlsToGroup(urls, targetGroupId, insertIndex) {
  ...
  if (targetGroup) {
    ...
    if (insertIndex >= 0 && insertIndex <= ungroupedUrls.length) {
      const targetUrl = ungroupedUrls[insertIndex];
      let targetPosInImageURLs = targetUrl
        ? imageURLs.indexOf(targetUrl)
        : imageURLs.length;
...
function updateSelectionBar() {
  ...
  if (globalDeselectAll) {
    ...
    globalDeselectAll.title = noneSelected
      ? "No images selected"
      : "Deselect all images";
  }
...
function updateGroupActionButtons() {
  groups.forEach(function (group) {
    const selectAllBtn = document.querySelector(
      '[data-action="select-all"][data-group-id="' + group.id + '"]'
    );
    const deselectBtn = document.querySelector(
      '[data-action="deselect"][data-group-id="' + group.id + '"]'
    );
    ...
    if (deselectBtn) {
      ...
      deselectBtn.title = noneSelected
        ? "No images selected"
        : "Deselect all images in group";
    }
  });
...
function updateUngroupedActionButtons() {
  ...
  const selectedInUngrouped = ungrouped.filter(function (url) {
    return selectedUrls.has(url);
  }).length;
  const isEmpty = ungroupedCount === 0;
  const allSelected =
    ungroupedCount > 0 && selectedInUngrouped === ungroupedCount;
  ...
  if (selectAllBtn) {
    ...
    selectAllBtn.title = isEmpty
      ...
      : allSelected
      ? "All images already selected"
      : "Select all ungrouped images";
  }
  if (deselectBtn) {
    deselectBtn.disabled = noneSelected;
    deselectBtn.title = noneSelected
      ? "No images selected"
      : "Deselect all ungrouped images";
  }
  if (removeAllBtn) {
    removeAllBtn.disabled = isEmpty;
    removeAllBtn.title = isEmpty
      ? "No images to remove"
      : "Remove all ungrouped images";
  }
}
...
function showNewGroupModal() {
  ...
  const existingGroupsSection = document.getElementById(
  ...
  function cleanup() {
    ...
    dialogTitle.innerHTML =
      'Add <span id="add-to-group-count">0</span> images to group';
...
function setupEventListeners(statusDiv) {
  ...
  document
    ...
    .addEventListener("click", createGroupFromSelection);

  // Global selection buttons
  document
    .getElementById("global-select-all")
    .addEventListener("click", selectAllGlobal);
  document
    .getElementById("global-deselect-all")
    .addEventListener("click", deselectAll);

  // Add empty group button
  document
    .getElementById("add-group")
    .addEventListener("click", createEmptyGroup);
  ...
  document.getElementById("flush").addEventListener("click", function () {
    ...
    showConfirmModal(
      ...
      function () {
        browserAPI.storage.local.set(
          { navigationStack: [], groups: [], imageMeta: {} },
          function () {
            imageURLs = [];
            selectedUrls.clear();
            groups = [];
  ...
  document
    .getElementById("ungrouped-select-all")
    .addEventListener("click", selectAllUngrouped);
  document
    .getElementById("ungrouped-deselect")
    .addEventListener("click", deselectAllUngrouped);

  // Ungrouped path edit button
  document
    .getElementById("ungrouped-path-edit")
    .addEventListener("click", function (e) {
      e.stopPropagation();
      showDirectoryEditModal(null, settings.ungroupedDirectory || "");
    });
  document
    .getElementById("ungrouped-remove-all")
    .addEventListener("click", function () {
      const ungroupedUrls = getUngroupedUrls();
      if (ungroupedUrls.length === 0) return;
      showConfirmModal(
        ...
        "Remove all " +
          ungroupedUrls.length +
          " ungrouped images? This cannot be undone.",
        removeAllUngrouped
      );
    });
      ...
      function () {
        ...
        if (done === total) {
          if (failed > 0) {
            ...
            showStatus(
              statusDiv,
              "Downloaded " + completed + " images successfully!",
              "success"
            );
...
function setupFloatingTooltips() {
  ...
  if (!tooltip || !window.FloatingUIDOM) {
    ...
    document
      .querySelectorAll(".info-icon[data-tooltip]")
...
function detectMediaType(url, mimeType) {
  ...
  if (url) {
    ...
    if (
      /\.(jpg|jpeg|png|gif|webp|svg|bmp|ico|avif)(\?|$)/i.test(lower) ||
  ...
  if (html) {
    ...
    svgs.forEach((svg) => {
      ...
      addItem({
        ...
        mediaType: MEDIA_TYPES.IMAGE,
        format: "svg",
        hint: isLikelyUiElement(dataUrl, dimensions)
          ? ITEM_HINTS.UI_ELEMENT
          : ITEM_HINTS.UNKNOWN,
        dimensions: dimensions,
      });
...
function analyzeAndSortItems(items) {
  ...
  const byDedupeKey = groupBy(
    items.filter((i) => i.dedupeKey),
    "dedupeKey"
  );
  ...
  return {
    ...
    recommended: items.filter(
      (i) => i.hint === ITEM_HINTS.PRIMARY || i.hint === ITEM_HINTS.UNKNOWN
    ),
...
function setupExternalDropZone() {
  ...
  function validateDragData(e) {
    ...
    return {
      valid: false,
      type: "unknown",
      message: "Cannot add this content",
    };
  }
      ...
      if (validation.valid) {
        if (targetGroupId) {
          ...
          dropText.textContent =
            'Drop to add to "' + (group ? group.name : "group") + '"';
        } else {
...
function setupHeaderIcons() {
  ...
  const globalDeselectAll = document.getElementById("global-deselect-all");
  if (globalSelectAll)
    Icons.setElementIcon(globalSelectAll, "check-circle", { size: iconSizeSm });
  if (globalDeselectAll)
    Icons.setElementIcon(globalDeselectAll, "x-circle", { size: iconSizeSm });

  // Add Group button (folder-plus icon to mean "create new group")
  const addGroupBtn = document.getElementById("add-group");
  if (addGroupBtn) {
    addGroupBtn.innerHTML = "";
    addGroupBtn.appendChild(
      Icons.createIcon("folder-plus", { size: iconSizeSm })
    );
  ...
  if (ungroupedSelectAll)
    Icons.setElementIcon(ungroupedSelectAll, "check-circle", {
      size: iconSizeSm,
    });
  if (ungroupedDeselect)
    Icons.setElementIcon(ungroupedDeselect, "x-circle", { size: iconSizeSm });
  if (ungroupedRemoveAll)
    Icons.setElementIcon(ungroupedRemoveAll, "trash", { size: iconSizeSm });
  if (ungroupedPathEdit)
    Icons.setElementIcon(ungroupedPathEdit, "pencil", { size: iconSizeXs });
  ...
  if (createGroupBtn) {
    ...
    createGroupBtn.appendChild(
      Icons.createIcon("arrow-right-end-on-rectangle", { size: iconSizeSm })
    );
    const span = document.createElement("span");
  ...
  document.querySelectorAll(".info-icon[data-tooltip]").forEach(function (el) {
    ...
    el.appendChild(
      Icons.createIcon("question-mark-circle", {
        size: iconSizeSm,
        className: "info-icon",
      })
    );
  ...
  if (undockIcon) {
    ...
    undockIcon.appendChild(
      Icons.createIcon("arrow-top-right-on-square", { size: iconSizeSm })
    );
  }
}
  ...
  document.addEventListener(
    ...
    function (e) {
      if (
        e.target &&
        e.target.classList &&
        e.target.classList.contains("filename-input")
      ) {
